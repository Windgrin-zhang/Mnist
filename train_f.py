...
train_dataset = torchvision.datasets.FashionMNIST(root='./fashionmnist', train=True, transform=transform, download=True)
test_dataset  = torchvision.datasets.FashionMNIST(root='./fashionmnist', train=False, transform=transform, download=True)

# 你自己的jpg图片（如需用作测试集，取消下行注释）
#test_dataset = FashionMNISTJPG('./fashionmnist_jpg', transform=transform)
...
for idx, img in enumerate(examples):
    ...
    plt.axis('off')
plt.suptitle('LeNet FashionMNIST预测结果可视化')
plt.tight_layout()
...
if __name__ == '__main__':
    ...
    Y = model(X)
    make_dot(Y, params=dict(model.named_parameters())).render("output_f/lenet_graph", format="pdf")
